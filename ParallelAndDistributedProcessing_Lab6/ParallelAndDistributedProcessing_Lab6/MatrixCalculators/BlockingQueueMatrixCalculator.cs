...
{
    public class BlockingQueueMatrixCalculator:IMatrixCalculator
    {
        ...
        public Matrix<double> MultiplyByMatrix(Matrix<double> matrixA, Matrix<double> matrixB)
        {
            ...
            BlockingCollection<double[,]>[,] queuesA= new BlockingCollection<double[,]>[blockCount, blockCount];
            BlockingCollection<double[,]>[,] queuesB = new BlockingCollection<double[,]>[blockCount, blockCount];

            for (int i = 0; i < blockCount; i++)
            {
                for (int j = 0; j < blockCount; j++)
                {
                    var blocksAQueue = new BlockingCollection<double[,]>(new ConcurrentQueue<double[,]>());
                    var blocksBQueue = new BlockingCollection<double[,]>(new ConcurrentQueue<double[,]>());

                    queuesA[i, j] = blocksAQueue;
                    queuesB[i, j] = blocksBQueue;
                    ...
                    Thread thread = new Thread(() =>
                    {
                        double[,] blockResult = MultiplyBlocks(blocksAQueue, blocksBQueue, size, blockCount);
                        lock (resultValues)
                        {
            ...
            for (int s = 0; s < blocksA.GetLength(0); s++)
            {
                for (int i = 0; i < blockCount; i++)
                {
                    for (int j = 0; j < blockCount; j++)
                    {
                        ...
                        queuesA[i, j].Add(blocksA[i, (i + s) % blocksA.GetLength(0)]);
                        queuesB[i, j].Add(blocksB[(i + s) % blocksA.GetLength(0), (j + s) % blocksB.GetLength(0)]);
                    }
                }
            }
        ...
        private double[,] MultiplyBlocks(BlockingCollection<double[,]> blocksA, BlockingCollection<double[,]> blocksB, int size, int blocksCount)
        {
            double[,] result = new double[size, size];

            for (int s = 0; s < blocksCount; s++)
            {
                var matrixA = blocksA.Take();
                var matrixB = blocksB.Take();
